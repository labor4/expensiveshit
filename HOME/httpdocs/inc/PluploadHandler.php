...
class PluploadHandler
{
    ...
    function handleUpload()
    {
        ...
        try {
			...
			$this->chosenpath = $this->getTargetPathFor($file_name, true);
            ...
            $result['finalpath'] = $this->chosenpath;
    ...
    {
        return call_user_func($this->conf['cb_filesize'], $this->chosenpath);
    }
     ...
     */
    function getTargetPathFor($file_name,$checkexists = false)
    {
        $target_dir = str_replace(array("/", "\/"), DS, rtrim($this->conf['target_dir'], "/\\"));
        if ($checkexists){
	        return $this->file_checkexists($target_dir, $file_name);
        } else {
	...
	protected function file_checkexists($path, $filename){
